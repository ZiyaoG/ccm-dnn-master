...
x_xhat_u_d_0 = [x0;x0;controller.u_nom_fcn(0);ue(end);[0;0];[0;0];[1;0;zeros(4,1)]]; % state, input, energy, true disturance,estimated disturbance;
...
xaTraj = x_xhat_u_d_Traj(2*n+8:3*n+7,:);
...
h3 = plot(xaTraj(1,:),xaTraj(2,:),'b-.','Linewidth',1.5);
...
xlabel('p_x (m)')
ylabel('p_z (m)')
legend([h1,h2,h3],{'Planned','RD-CCM','Adaptive CCM'});
xlim([-1 11]);
ylim([-1 11]);
w_max = 1;
...
xa = x_xhat_u_d(2*n+8:3*n+7);
% xanom = x_xhat_u_d(3*n+8:4*n+7);
...
% ------------------- adaptive ccm -----------------------------
ua = adaptive(t,x,plant,controller,distLearned,distEst_ccm_control,distEst_config.est_errBnd,dist_config,sim_config);
u_a = ua(1:end-1);

theta = [0.5;0.5];
phi = (xa(4)^2+xa(5)^2)/((xa(1)-dist_config.center(1))^2+(xa(2)-dist_config.center(2))^2+1);
% xanomdot = plant.f_fcn(x)+plant.B_fcn(x)*u_a;
xadot = plant.f_fcn(x)+plant.B_fcn(x)*(u_a - phi*theta);
% ------------------------------------------------------------------------

% update the states of actual system, state predictor, ...
dxdt = [xdot; xhatdot; -Klp.*u_e_dist_estDist_filter; xadot]+[zeros(n,1); zeros(n,1); Klp.*[ue;wt;distEst];zeros(n,1)];
if sim_config.include_dist == 1
