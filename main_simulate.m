...
plot(times,xaTraj(1,:),'b.',times,xaTraj(2,:),'r.');
xlabel('Time (s)')
ylabel('x & z (m)')
legend('x_{nom}', 'z_{nom}', 'x_{adpt}', ['x: ' addText],['z: ' addText], ['xa: ' addText]);
...
persistent distEst Bsigmahat thetahat
if isempty(distEst)
    distEst = [0;0];
    Bsigmahat = zeros(6,1);
    thetahat = [0;0];
...
gamma_s1 = xa-controller.x_nom_fcn(t);
dthetahat = -Gamma*phi*plant.B_fcn(xa)'*controller.M*gamma_s1;
thetahat = thetahat + dthetahat*sim_config.step_size;
xadot = xnomdot - plant.B_fcn(xa)*phi*thetahat;
% ------------------------------------------------------------------------



% update the states of actual system, state predictor, ...
dxdt = [xnomdot; xhatdot; -Klp.*u_e_dist_estDist_filter; xadot]+[zeros(n,1); zeros(n,1); Klp.*[ue;wt;distEst]; zeros(n,1)];
if sim_config.include_dist == 1
